...
def expire_account():
    ...
    for user in get_user_model().objects.select_related('userplan').filter(userplan__active=True,         userplan__expire__lt=datetime.date.today()).exclude(userplan__expire=None):
